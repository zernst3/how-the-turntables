...
const seed = async () => {
  try {
    ...
    for (let i = 0; i < orderArr.length; i++) {
      await orderArr[i].addProduct(
        productArr[chance.integer({min: 1, max: 199})]
      )
    }

    // orderArr.forEach(async (order) => {
    //   await order.addProduct(productArr[chance.integer({min: 1, max: 199})])
    // })
  } catch (err) {
    console.log(red(err))
  }
